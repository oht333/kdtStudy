...
public class BookDAO {
   ...
   public List<BookDTO> selectAllBook() {
      String query = "SELECT book_number, book_title, book_author, book_rent " + "FROM tbl_book";
...
//                     System.out.println("데베 값 : "+Boolean.valueOf(resultSet.getString(4));
            bookRent = Boolean.valueOf(resultSet.getString(4)); // resultSet.getString(4)이 문자열이라 이를 Boolean으로 변환해서
                                                   // 참/거짓으로 변환
         }
      } catch (SQLException e) {
         e.printStackTrace();
